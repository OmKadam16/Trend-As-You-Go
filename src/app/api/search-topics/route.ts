...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `You are a trend analysis assistant. Identify current trending discussion and topics on X (Twitter) specifically related to "${query.trim()}".
...
Return up to 10 results. If nothing relevant is trending, return an empty array. Return ONLY a valid JSON array, no other text.`;
  ...
  try {
      ...
      "https://api.openai.com/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: "user",
              content: `What's trending on X right now related to ${query}?`,
            },
          ],
          max_tokens: 4096,
          temperature: 0.7,
    ...
    if (!content) {
      return NextResponse.json(
        {
          error:
            "Something went wrong searching trends. Try again.",
        },
        { status: 500 }
      );
    }
